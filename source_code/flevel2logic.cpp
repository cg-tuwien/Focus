...
void flevel2logic::initialize() {
	...
	auto mirrorBorderInstance = scene->get_model_by_name("Cube.002");//MirrorBorder1
	if (!mirrorBorderInstance.has_value()) throw new std::runtime_error("Did not find MirrorBorder1(Cube.002)");
...
levelstatus flevel2logic::update(float deltaT, double focusHitCount)
{
	/*if (score > 2.0f) {
		return WON;
	}
